...
/**
 * @author chenpc
 * @version $Id: localTest.TestGeneric.java, v 0.1 2018-05-23 17:43:58 chenpc Exp $
 */
public class TestGeneric<T> {
    ...
    private <T extends Fruit> void test1( T s, List<? super Apple> l) {  //泛型方法的参数必须声明 <T>
        ...
        l.add(new Apple());   // 例：List<? super localTest.Apple> 可能指向List<localTest.Fruit> , get时可能会获取一个Fruit，不可由Apple接收
                              // add 一个Apple可被 List<localTest.Fruit>接收  <==>  l.get()无法使用使用一个具体的类接收，l内均为apple的父类
        System.out.println(s);
    }

    private <T> void test2(T s, List<? extends Fruit> l) {
        //List<? extends Object> -->producer get -->PE  --> 从集合中读取类型T的数据，并且不能写入
        l.get(0);   // 例：List<? extends localTest.Fruit>可能指向List<localTest.Apple>/List<localTest.Orange>/List<localTest.Fruit>, add 一个Apple可能被List<localTest.Orange>接收 所以不可使用add
                    // get则可由父类 Fruit接收
        System.out.println(s);
    }
